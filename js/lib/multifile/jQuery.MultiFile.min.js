	...
	$.fn.MultiFile = function (options) {
		...
		this
			...
			.each(function () {
				...
				$(".modal-body-"+MultiFile.wrapID).append(MultiFile.list); // changed by Jhalak Mehta on Date : 11/01/2016 [to show file name in Lightbox popup]
				//===
				...
				MultiFile.addToList = function (slave, slave_count, files) {
					...
					$.each(files, function (i, file) {
						...
						var v = String(file.name || '' ),
								...
								it = '<input type="hidden" class="hidden-preview" value="" name="'+$(MultiFile.clone).attr('name')+'" data-file="true" data-title="$file"/>',
								label =	$(
										(
											'<span class="MultiFile-label" title="' + t + '">'+
												'<span class="MultiFile-title">'+ n +'</span>'+
												(MultiFile.preview || $(slave).is('.with-preview') ? p : '' )+
												(MultiFile.preview || $(slave).is('.with-preview') ? it : '' )+
											'</span>'
										)
						...
						//store data in hidden input to get it while form submit (by Jhalak - 08/01/2016)
						label.find('input.hidden-preview').each(function(){
							var t = this;
							var oFReader = new FileReader();
							oFReader.readAsDataURL(file);
							oFReader.onload = function (oFREvent) {
								$(t).val(oFREvent.target.result);
							};
						});
